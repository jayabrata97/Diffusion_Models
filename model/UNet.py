...
class MidBlock(nn.Module):
    def __init__(
        self, in_channels, out_channels, t_emb_dim, num_heads=4, num_layers=1
    ) -> None:
        super().__init__()
        self.num_layers = num_layers
        self.resnet_conv_first = nn.ModuleList(
...
class UNet(nn.Module):
    def __init__(self, model_config) -> None:
        super().__init__()
        pic_channels = model_config["pic_channels"]
        ...
        self.conv_in = nn.Conv2d(
            pic_channels, self.down_channels[0], kernel_size=3, padding=(1, 1)
        )
        self.downs = nn.ModuleList([])
        for j in range(len(self.down_channels) - 1):
            self.downs.append(
                DownBlock(
